...
async def debug_competitor_extraction(url: str, description: str):
    """调试跟卖店铺提取"""
    print(f"\n{'='*80}")
    ...
    try:
        ...
        # 检查跟卖区域
        competitor_element = await page.query_selector(precise_selector)
        if competitor_element:
            is_visible = await competitor_element.is_visible()
            text_content = await competitor_element.text_content()
            print(f"✅ 找到跟卖区域，可见性: {is_visible}")
            print(f"📝 跟卖区域文本: {text_content[:100] if text_content else 'N/A'}")
            
            if is_visible:
                print("🔍 第二步：尝试打开跟卖浮层...")
                result = await competitor_scraper.open_competitor_popup(page)
                ...
                if result.get('popup_opened'):
                    ...
                    expand_success = await competitor_scraper.expand_competitor_list_if_needed(page)
                    ...
                    # 提取跟卖店铺信息
                    competitors = await competitor_scraper.extract_competitors_from_content(page_content, max_competitors=10)
...
@pytest.mark.asyncio
async def test_competitor_extraction_144042159():
    """测试URL 144042159 的跟卖信息提取"""
    await debug_competitor_extraction(
        "https://www.ozon.ru/product/144042159",
        "跟卖信息错误的商品"
    )

@pytest.mark.asyncio
async def test_competitor_extraction_2369901364():
...
async def main():
    ...
    for url, description in test_urls:
        await debug_competitor_extraction(url, description)
        print("\n" + "="*80 + "\n")

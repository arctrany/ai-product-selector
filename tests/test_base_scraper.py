...
class TestBaseScraper:
    ...
    def test_scrape_page_data_no_browser_service(self):
        """测试 scrape_page_data 缺少 browser_service - 应该直接抛异常"""
        # Arrange
        test_url = "https://example.com/test"
        # browser_service 是 None（在 __init__ 中初始化）

        # Act & Assert - 应该抛出 AttributeError
        result = self.scraper.scrape_page_data(test_url, self.mock_extractor)
    ...
    def test_multiple_scrape_calls(self):
        ...
        assert self.mock_browser_service.navigate_to_sync.call_count == 2
    ...
    def test_navigate_to_success(self):
        ...
        # Act
        result = self.scraper.navigate_to(test_url)
    ...
    def test_navigate_to_with_custom_wait_until(self):
        ...
        # Act
        result = self.scraper.navigate_to(test_url, wait_until="networkidle")

        # Assert
        assert result is True
        self.mock_browser_service.navigate_to_sync.assert_called_once_with(test_url, "networkidle")

    def test_navigate_to_failure(self):
        """测试 navigate_to 方法导航失败"""
        # Arrange
        test_url = "https://example.com/test"
        self.scraper.browser_service = self.mock_browser_service
        self.mock_browser_service.navigate_to_sync.return_value = False

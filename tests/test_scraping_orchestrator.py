...
class TestScrapingOrchestratorModes(unittest.TestCase):
    ...
    @patch('common.services.scraping_orchestrator.OzonScraper')
    def test_product_info_mode(self, mock_ozon_scraper_class):
        """测试商品信息抓取模式"""
        # 设置Mock
    ...
    @patch('common.services.scraping_orchestrator.SeerfarScraper')
    @patch('common.services.scraping_orchestrator.ScrapingUtils')
    def test_store_analysis_mode(self, mock_scraping_utils_class, mock_seerfar_scraper_class):
        """测试店铺分析抓取模式"""
        # 设置Mock
        ...
        mock_scraping_utils.extract_store_id_from_url.return_value = 'store_123'
    ...
    @patch('common.services.scraping_orchestrator.CompetitorDetectionService')
    @patch('common.services.scraping_orchestrator.CompetitorScraper')
    @patch('common.services.scraping_orchestrator.OzonScraper')
    def test_competitor_detection_mode(self, mock_ozon_scraper_class, mock_competitor_scraper_class, mock_detection_service_class):
        """测试跟卖检测模式"""
        # 设置Mock
        mock_ozon_scraper = Mock()
        mock_ozon_scraper_class.return_value = mock_ozon_scraper
    ...
    @patch('common.services.scraping_orchestrator.ErpPluginScraper')
    def test_erp_data_mode(self, mock_erp_scraper_class):
        """测试ERP数据抓取模式"""
        # 设置Mock
...
class TestScrapingOrchestratorFullAnalysis(unittest.TestCase):
    ...
    @patch('common.services.scraping_orchestrator.CompetitorDetectionService')
    @patch('common.services.scraping_orchestrator.CompetitorScraper')
    def test_full_analysis_mode(self, mock_competitor_scraper_class, mock_detection_service_class, 
...
class TestScrapingOrchestratorErrorHandling(unittest.TestCase):
    ...
    @patch('common.services.scraping_orchestrator.OzonScraper')
    def test_scraper_failure(self, mock_ozon_scraper_class):
        """测试Scraper失败情况"""
        # 设置Mock
        mock_ozon_scraper = Mock()
        mock_ozon_scraper_class.return_value = mock_ozon_scraper

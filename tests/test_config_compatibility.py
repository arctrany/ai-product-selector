...
class TestBackwardCompatibility:
    ...
    def test_store_filter_property_compatibility(self):
        """测试store_filter属性向后兼容性"""
        config = GoodStoreSelectorConfig()
        
        # 测试访问废弃的store_filter属性会产生警告
        with pytest.warns(DeprecationWarning, match="配置属性 'store_filter' 已废弃"):
            store_filter_config = config.store_filter
        
        # 验证返回的是selector_filter配置
        assert store_filter_config is config.selector_filter
        assert isinstance(store_filter_config, SelectorFilterConfig)
    ...
    def test_deprecated_properties_functionality(self):
        ...
        with warnings.catch_warnings():
            ...
            assert store_filter.store_min_sales_30days == 500000.0
            assert store_filter.store_min_orders_30days == 250
    ...
    def test_property_equivalence(self):
        ...
        with warnings.catch_warnings():
            ...
            assert config.store_filter is config.selector_filter
...
class TestConfigurationLoading:
    ...
    def test_old_format_config_loading(self):
        ...
        if 'store_filter' in old_config_data:
            for key, value in old_config_data['store_filter'].items():
        ...
        assert config.selector_filter.profit_rate_threshold == 25.0
    ...
    def test_mixed_format_config_loading(self):
        ...
        # 如果没有新格式，使用旧格式
        if 'store_filter' in mixed_config_data and not selector_loaded:
            for key, value in mixed_config_data['store_filter'].items():
                if hasattr(config.selector_filter, key):
                    setattr(config.selector_filter, key, value)
        ...
        assert config.selector_filter.store_min_sales_30days == 600000.0  # 来自selector_filter字段
...
class TestConfigurationMigration:
    ...
    def test_store_filter_to_selector_filter_migration(self):
        """测试store_filter到selector_filter的迁移"""
        # 创建使用旧字段的配置
        old_config = {
            'store_min_sales_30days': 1200000.0,
            'store_min_orders_30days': 400,
            'profit_rate_threshold': 28.0,
            'good_store_ratio_threshold': 25.0
        }
        
        # 创建新的SelectorFilterConfig实例
        new_filter_config = SelectorFilterConfig(**old_config)
        
        # 验证迁移成功
        assert new_filter_config.store_min_sales_30days == 1200000.0
        assert new_filter_config.store_min_orders_30days == 400
        assert new_filter_config.profit_rate_threshold == 28.0
        assert new_filter_config.good_store_ratio_threshold == 25.0
    ...
    def test_complete_config_migration(self):
        ...
        # 迁移过滤器配置
        if 'store_filter' in old_full_config:
            for key, value in old_full_config['store_filter'].items():
        ...
        assert new_config.selector_filter.store_min_sales_30days == 900000.0
        assert new_config.selector_filter.profit_rate_threshold == 23.0
...
class TestWarningsAndMigrationGuidance:
    ...
    def test_deprecation_warning_content(self):
        ...
        # 测试store_filter属性警告内容
        with pytest.warns(DeprecationWarning) as warning_info:
            _ = config.store_filter
        
        warning_message = str(warning_info[0].message)
        assert "store_filter" in warning_message
        assert "废弃" in warning_message
        assert "selector_filter" in warning_message

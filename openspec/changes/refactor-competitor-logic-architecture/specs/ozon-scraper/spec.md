...
### Requirement: 时序控制标准化
OzonScraper 应当使用统一的时序控制机制，移除所有硬编码等待，确保跨平台兼容性和系统稳定性。
...
### Requirement: 职责边界明确化
OzonScraper 应专注于基础商品信息抓取，移除跟卖检测相关逻辑，遵循单一职责原则。
...
### Requirement: 代码复用统一化
OzonScraper 应使用统一的抓取工具类，消除与其他抓取器的重复逻辑，提高代码一致性。
...
### Requirement: 接口标准化兼容
OzonScraper 应提供标准化的接口，同时保持向后兼容性，支持渐进式迁移。
...
### Requirement: 配置管理优化
OzonScraper 应使用重构后的配置管理系统，支持配置继承和优先级控制。

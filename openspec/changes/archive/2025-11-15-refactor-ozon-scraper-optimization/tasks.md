...
- [x] 1.3 Update callers to use `_extract_product_image_core()` directly

## 2. Remove Redundant Price Extraction Method
- [x] 2.1 Remove `_extract_price_data_from_content_sync()` method
- [x] 2.2 Update `scrape_product_prices()` to call `_extract_price_data_core()` directly
- [x] 2.3 Remove unused `is_async` parameter from `_extract_price_data_core()`

...
### Decision 4: 跟卖店铺数据处理
**选择**: 抓取跟卖数据但不输出到商品Excel

**理由**:
- 跟卖数据已保存在内存中（`product_result.competitor_stores`）
- select-goods 模式的核心目标是商品清单，不是店铺裂变
- 如需跟卖分析，可在后续流程中处理

**实现方式**:
- 保持 `_collect_competitor_stores()` 逻辑不变
- 商品写入时忽略 `competitor_stores` 字段
...
**决策**: 接受这个权衡，因为：
- 跟卖分析属于高级功能，可在后续版本中添加独立报告
- 商品清单的核心价值是快速选品，不是竞争分析

## Migration Plan

### Phase 1: 实现核心功能（本次变更）
1. 新增 `ExcelProductWriter` 类
2. 修改 `_update_excel_results()` 添加分支逻辑
3. 修改 CLI 参数验证

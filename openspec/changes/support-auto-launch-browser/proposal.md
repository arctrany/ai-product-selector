...
- **自动开启 CDP 调试端口**：`--remote-debugging-port=9222`
  - 方便后续连接和调试
  - 支持多个脚本连接同一浏览器实例
...
**关键改进**：
- ✅ 真正的"无需手动操作"（优先启动模式）
- ✅ Profile 锁定时提示用户关闭浏览器（确保使用正确的用户数据）
- ✅ 启动模式自动开启 CDP 调试端口（方便后续连接）
- ✅ 兼容手动启动的场景（降级到连接模式）
...
检测到 Profile 被占用
  ↓
提示用户：
"检测到浏览器 Profile 正在被使用
请关闭所有 Edge/Chrome 浏览器窗口
然后重新运行脚本"
  ↓
用户关闭浏览器
  ↓

...
from paginator import UniversalPaginator
...
class AutomationScenario:
    ...
    def __init__(self, request_delay: float = 2.0, debug_mode: bool = False):
        """
        第一步：场景初始化
    ...
    async def _step_3_6_validate_and_integrate_data(self, store_data: Dict[str, Any]):
        ...
        # 判断是否成功提取到关键数据
        has_store_data = any([store_data['sales_amount'], store_data['sales_volume'], store_data['daily_avg_sales']])
        has_product_data = len(store_data['products']) > 0

...
class TestBrowserService:
    ...
    def test_start_browser_success(self):
        """测试成功启动浏览器"""
        service = BrowserService(config=self.test_config_dict)
    ...
    def test_get_page_content_success(self):
        ...
        with patch('src_new.rpa.browser.browser_service.PlaywrightBrowserDriver') as mock_driver_class:
            ...
            result = service.get_page_content()

            assert result == "<html>Test Content</html>"
            mock_driver.page.evaluate.assert_called_once()

    def test_get_page_content_no_browser(self):
        """测试没有浏览器时获取页面内容"""
        service = BrowserService(config=self.test_config_dict)
    ...
    def test_update_config_success(self):
        ...
        result = service.update_config('debug_mode', False)
    ...
    def test_close_success(self):
        ...
        with patch('src_new.rpa.browser.browser_service.PlaywrightBrowserDriver') as mock_driver_class:
            mock_driver = MagicMock()
            mock_driver.initialize = MagicMock(return_value=True)
            mock_driver.shutdown = MagicMock(return_value=None)
            mock_driver_class.return_value = mock_driver
    ...
    def test_analyze_page_success(self):
        ...
        with patch('src_new.rpa.browser.browser_service.PlaywrightBrowserDriver') as mock_driver_class:
            ...
            with patch('src_new.rpa.browser.implementations.dom_page_analyzer.OptimizedDOMPageAnalyzer') as mock_analyzer_class:
                ...
                mock_analyzer.analyze_page = MagicMock(return_value={'elements': [], 'links': []})
...
class TestBrowserServiceErrorHandling:
    ...
    def test_initialize_with_driver_failure(self):
        """测试驱动初始化失败"""
        service = BrowserService()

        with patch('src_new.rpa.browser.browser_service.PlaywrightBrowserDriver') as mock_driver_class:
            mock_driver = MagicMock()
            mock_driver.initialize = MagicMock(side_effect=Exception("Driver init failed"))
            mock_driver_class.return_value = mock_driver
    ...
    def test_navigate_with_driver_failure(self):
        """测试导航时驱动失败"""
        service = BrowserService()
        ...
        with patch('src_new.rpa.browser.browser_service.PlaywrightBrowserDriver') as mock_driver_class:
            mock_driver = MagicMock()
            mock_driver.initialize = MagicMock(return_value=True)
            mock_driver.open_page = MagicMock(side_effect=Exception("Navigation failed"))
            mock_driver_class.return_value = mock_driver
    ...
    def test_close_with_driver_failure(self):
        """测试关闭时驱动失败"""
        service = BrowserService()
        ...
        with patch('src_new.rpa.browser.browser_service.PlaywrightBrowserDriver') as mock_driver_class:
            mock_driver = MagicMock()
            mock_driver.initialize = MagicMock(return_value=True)
            mock_driver.shutdown = MagicMock(side_effect=Exception("Shutdown failed"))
            mock_driver_class.return_value = mock_driver

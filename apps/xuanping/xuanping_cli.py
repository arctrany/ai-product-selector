...
使用方式:
    python xuanping_cli.py start --config config.json    # 启动任务
    python xuanping_cli.py pause                         # 暂停任务
    python xuanping_cli.py resume                        # 恢复任务
    python xuanping_cli.py stop                          # 停止任务
    python xuanping_cli.py status                        # 查看状态
    python xuanping_cli.py config list                   # 列出配置
    python xuanping_cli.py config set key=value          # 设置配置
    python xuanping_cli.py logs                          # 查看日志
    python xuanping_cli.py progress                      # 查看进度
"""
...
class XuanpingCLIController:
    ...
    def __init__(self):
        self.config_dir = Path.home() / ".xuanping" / "configs"

...
class PlaywrightBrowserDriver(IBrowserDriver):
    ...
    def _get_minimal_launch_args(self, profile_name: str, enable_extensions: bool) -> List[str]:
        ...
        args = [
            ...
            '--disable-infobars',  # 禁用信息栏（包括"被控制"提示）
            '--disable-web-security',  # 禁用Web安全限制
            '--disable-features=VizDisplayCompositor',  # 禁用可能触发检测的功能
            '--exclude-switches=enable-automation',  # 排除自动化开关
            '--disable-automation',  # 禁用自动化标识
    ...
    async def _inject_stealth_scripts(self, page) -> None:
        ...
        try:
            ...
            // 9. 重写Date.prototype.getTimezoneOffset
            ...
            // 12. 重写toString方法以隐藏native code标识
            ...
            EventTarget.prototype.addEventListener = function(type, listener, options) {
                if (type === 'beforeunload' && listener.toString().includes('automation')) {
                    return;
                }
            ...
            self._logger.debug("✅ 反检测脚本注入成功")

        except Exception as e:
            self._logger.warning(f"⚠️ 反检测脚本注入失败: {e}")
